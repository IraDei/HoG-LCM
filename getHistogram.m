...
% HOG feature extraction function provided by 'chrisjmccormick', origin
% project on git-hub: https://github.com/chrisjmccormick/hog_matlab
%
...
for i = 1:numBins
    ...
    % there is no difference without 'sum' on matrice product with size
    % 1*9*9*1, whose result is a 1*1 scalar. (In R2018b)
    H(1, i) = H(1, i) + sum(leftPortions(pixels)' * magnitudes(pixels));
    %H(1, i) = H(1, i) + leftPortions(pixels)' * magnitudes(pixels);
    ...
    %H(1, i) = H(1, i) + rightPortions(pixels)' * magnitudes(pixels);
end    
